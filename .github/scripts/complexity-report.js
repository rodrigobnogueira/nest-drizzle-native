...
module.exports = async ({ github, context }) => {
  ...
  const base = hasBase
    ? JSON.parse(fs.readFileSync('base-cognitive-complexity-summary.json', 'utf8'))
    : null;
  ...
  const topEntries = current.entries.slice(0, 10).map(entry => {
    const symbol = entry.symbol ?? '(unknown function)';
    return `| \`${symbol}\` | \`${entry.file}:${entry.line}\` | ${entry.complexity} | ${status(entry.complexity)} |`;
  });
